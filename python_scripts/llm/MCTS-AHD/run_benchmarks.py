#!/usr/bin/env python
"""Score every MCTS-AHD heuristic on the held-out ATSP test set.

Walks `runs/llm/MCTS-AHD/<task>-<type>/*/best_heuristic.py`, evaluates each one
plus each task's seed heuristic, and delegates to eval_mcts_ahd_atsp.py — one
subprocess per evaluation, so a heuristic that hangs or crashes can never abort
the sweep.

    python python_scripts/llm/MCTS-AHD/run_benchmarks.py                # seeds + all runs
    python python_scripts/llm/MCTS-AHD/run_benchmarks.py --baselines-only
    python python_scripts/llm/MCTS-AHD/run_benchmarks.py --runs-only
    python python_scripts/llm/MCTS-AHD/run_benchmarks.py --task atsp_gls --max-n 100
...
from evaluation.llm.mcts_ahd import TASKS  # noqa: E402

HERE = os.path.dirname(os.path.abspath(__file__))
EVAL = os.path.join(HERE, "eval_mcts_ahd_atsp.py")
...
def main(argv=None) -> int:
    ...
    for task in tasks:
        ...
        found = sorted(glob.glob(os.path.join(RUNS, f"{task}-*", "*",
                                              "best_heuristic.py")))
        if not found:
            print(f"  (no finished MCTS-AHD runs for task '{task}')")
        for path in found:
            failures += bool(_run([sys.executable, EVAL,
                                   "--run", os.path.dirname(path), *extra]))

    print("\nNow build the tables:")
    print("  python python_scripts/llm/MCTS-AHD/generate_paper_tables.py")
