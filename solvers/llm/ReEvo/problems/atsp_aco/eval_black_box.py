"""Black-box view of the ATSP ACO task.

Identical evaluation to `eval.py`; the only difference is the interface the LLM
sees. Here the heuristic receives a flat `(n_edges, n_attributes)` array of edge
attributes and returns `(n_edges,)` scores, with no hint that the problem is a
routing problem at all — ReEvo's black-box setting.

    python -u eval_black_box.py <problem_size> <root_dir> <train|val|test>
"""
...
def solve(instance, n_iterations: int, time_limit: float) -> float:
    ...
    edge_attr = dist.reshape(-1, 1).copy()
...
if __name__ == "__main__":
    print("[*] Running black-box ATSP ant colony optimisation ...")
