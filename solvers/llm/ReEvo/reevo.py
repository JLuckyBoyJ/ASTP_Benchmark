...
class ReEvo:
    ...
    def _run_code(self, individual: dict, response_id) -> subprocess.Popen:
        ...
        with open(individual["stdout_filepath"], 'w',encoding="utf-8") as f:
            ...
            # sys.executable, not the bare name "python": on macOS and many
            # Linux distros only `python3` exists on PATH, and even where
            # `python` exists it may be a different interpreter than the one
            # running ReEvo (wrong venv => missing numpy, wrong package set).
