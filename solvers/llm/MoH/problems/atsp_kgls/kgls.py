...
-------------------------------------------
Both of MoH's improvement tasks hand the LLM a callable rather than a matrix,
so the difference here is narrower than it is on the MCTS-AHD side — but it is
still a difference in *what* is designed. ``atsp_gls`` asks for a rule that
returns an updated ``(n, n)`` cost matrix, which the search then ranks arcs by;
this task asks for a rule that scores the arcs of the current tour directly,
and additionally sees the iteration index, so it can schedule its own
exploration. The engine budget is held identical between the two
(``configs/llm/MoH/cfg/evaluation/atsp_{gls,kgls}.yaml``), so a gap difference
between them is a difference in the design space and nothing else.
