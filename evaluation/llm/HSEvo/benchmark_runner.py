...
for _p in (ROOT, HSEVO, os.path.join(HSEVO, "problems", "atsp_gls"),
           os.path.join(HSEVO, "problems", "atsp_aco")):
...
def seed_heuristic(task: str) -> str:
    """HSEvo's own seed function — the baseline every task must beat."""
    path = os.path.join(HSEVO, "prompts", task, "seed_func.txt")
    with open(path, encoding="utf-8") as fh:
        return "import numpy as np\n\n" + fh.read().replace("_v1(", "(")


def load_heuristic_for_run(run_dir: str) -> tuple[str, str]:
    """Return ``(task, source_code)`` for a finished HSEvo run directory."""
